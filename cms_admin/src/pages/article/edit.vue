  ...
  <div class="article-edit">
    <el-form label-position='right' size='small' label-width="100px" class="article-form">
      ...
      <el-form-item v-for="(item,index) in articleFields" :key='index' :label="item.cname" v-if='item.inputType != "auto"'>
        ...
        <el-upload-extr v-else-if='item.inputType == "imgfile"' :file.sync="article.fields[item.name]" :limit='1' accept='image/jpeg,image/png'>
          <el-button size="small" type="primary">点击上传</el-button>
          <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>
        </el-upload-extr>
        <el-upload-extr v-else-if='item.inputType == "imgfiles"' :file.sync="article.fields[item.name]" accept='image/jpeg,image/png'>
          <el-button size="small" type="primary">点击上传</el-button>
          <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>
        </el-upload-extr>
        <el-upload-extr v-else-if='item.inputType == "videofile"' :file.sync="article.fields[item.name]" :limit='1' accept='video/mp4,video/x-msvideo,video/quicktime'>
          <el-button size="small" type="primary">点击上传</el-button>
          <div slot="tip" class="el-upload__tip">只能上传mp4/avi文件，且不超过100mb</div>
        </el-upload-extr>
        <el-upload-extr v-else-if='item.inputType == "videofiles"' :file.sync="article.fields[item.name]" accept='video/mp4,video/x-msvideo,video/quicktime'>
          <el-button size="small" type="primary">点击上传</el-button>
          <div slot="tip" class="el-upload__tip">只能上传mp4/avi文件，且不超过100mb</div>
        </el-upload-extr>
